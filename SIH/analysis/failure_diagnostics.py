...
def diagnose_registration_run(
    ...
    # 5. Overlap & Metadata Integrity Check
    if overlap_report.level.startswith("Level 3") or overlap_report.confidence == "UNKNOWN":
        findings.append(DiagnosticFinding(
            category="LOW_SPATIAL_OVERLAP",
            severity="WARNING",
            observation="Metadata limitation: Input images lack georeferenced PDS4/GeoTIFF spatial footprints. Coarse phase correlation peak was insufficient to verify geographic overlap.",
            recommendation="Registration results are exploratory only. Provide calibrated PDS4 or GeoTIFF products with embedded map projection coordinates to establish ground-truth geographic intersection.",
        ))

    # 6. Final Verified Inlier Count Check
    effective_inliers = metrics.final_inlier_count if metrics is not None else match_result.n_inliers
    if effective_inliers < 4:
        findings.append(DiagnosticFinding(
            category="INSUFFICIENT_MATCHES",
            severity="CRITICAL",
            observation=f"Only {effective_inliers} final verified correspondence(s) remained (minimum 4 required for planar transformation).",
            recommendation="Images lack sufficient common visual features. Check geographic footprints, compare physical GSD, or test with known-overlap regions.",
        ))
        rec_params["matcher"] = "hybrid"

    # 7. Spatial Coverage Check
    if metrics and metrics.spatial_coverage_pct < 25.0 and effective_inliers >= 4:
        findings.append(DiagnosticFinding(
    ...
    if metrics is not None:
        is_success = (
            effective_inliers >= 4
            and metrics.rmse_px is not None
            and metrics.rmse_px < 4.0
            and not any(f.severity == "CRITICAL" for f in findings)
        )
    else:
        is_success = effective_inliers >= 4 and not any(f.severity == "CRITICAL" for f in findings)
