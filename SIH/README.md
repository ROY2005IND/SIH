...
**Engine:** Offline-First Planetary Cartography Workstation (MoonFlower Core Engine)
...
**LUNARMATCH AI** is an offline-first, research-grade planetary image registration platform built around three foundational layers:
...
- **Layer 2 — Registration Engine:** Pre-matching radiometric and textural image quality checks, tiered 3-level overlap estimation, illumination-invariant representations (CLAHE, Sobel gradient, 2D phase congruency, IIRS PCA), Adaptive Hybrid correspondence engine with $O(N \log N)$ `cKDTree` spatial deduplication, Occam's razor geometric model selection, Adaptive Spatial Match Balancer ($4 \times 4, 6 \times 6, 8 \times 8$), and genuine 2D parabolic sub-pixel peak optimization.
- **Layer 3 — Scientific Integrity & Verification:** Mathematically grounded evaluation metrics with strict degrees-of-freedom validation ($N \ge 4$ required; never fabricates 0.000 px RMSE on degenerate inlier sets), three explicit operating states (`VERIFIED_GEOSPATIAL`, `IMAGE_ONLY_EXPLORATORY`, `REJECTED`), rule-based failure diagnostics, side-by-side Benchmark Lab, and full scientific export packaging (CSV, JSON, Markdown report).
...
# 5. Run full automated test suite (95+ unit, integration, and regression tests)
python -m pytest tests/ -v

# 6. Launch Mission Control Planetary Workstation
