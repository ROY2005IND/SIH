...
def compute_spatial_uniformity(
    ...
    norm_entropy = float(entropy / max_entropy) if max_entropy > 0 and occupied_cells > 1 else 0.0
    uniformity_score = round(max(0.0, float(occupancy_ratio * norm_entropy * 100.0)), 2)
    occupancy_pct    = round(max(0.0, occupancy_ratio * 100.0), 2)
