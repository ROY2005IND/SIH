...
def test_learned_extractor_honesty_without_weights():
    from features.learned import LearnedFeatureExtractor, LearnedFeatureOutput
    extractor = LearnedFeatureExtractor("superpoint")
    assert extractor.model_loaded is False

    synthetic_img = np.random.randint(50, 200, (128, 128), dtype=np.uint8)
    feat_out = extractor.extract(synthetic_img)
    assert isinstance(feat_out, LearnedFeatureOutput)
    assert feat_out.is_deep_model is False
    assert feat_out.backend_status in ("CLASSICAL_FALLBACK", "UNAVAILABLE")


def test_learned_matcher_honesty():
    from matching.learned import LearnedMatcher
    matcher = LearnedMatcher(model_name="superpoint")
    assert matcher.is_deep_model is False
    assert matcher.backend_status == "CLASSICAL_FALLBACK"


def test_hybrid_matcher_diagnostic_honesty():
    hybrid = AdaptiveHybridMatcher()
    synthetic_img1 = np.random.randint(50, 200, (200, 200), dtype=np.uint8)
    synthetic_img2 = synthetic_img1.copy()
    _, _, _, _, diag = hybrid.find_matches(synthetic_img1, synthetic_img2)

    assert diag.is_deep_model is False
    assert diag.secondary_backend == "classical_gftt_sift_fallback"
