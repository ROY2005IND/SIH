...
def test_model_selection_engine():
    ...
    assert model in ["translation", "similarity", "affine", "homography"]
...
def test_pure_translation_selected():
    rng = np.random.default_rng(42)
    src_pts = rng.uniform(30.0, 180.0, (50, 2)).astype(np.float32)
    true_M = np.array([[1.0, 0.0, 15.0], [0.0, 1.0, -22.0]], dtype=np.float64)
    ref_pts = apply_transformation(src_pts, true_M)

    model, M, mask, res, reason, evals = select_best_transformation_model(
        src_pts, ref_pts,
        candidate_models=("translation", "similarity", "affine", "homography"),
    )
    assert model == "translation"
    assert np.allclose(M, true_M, atol=0.01)


def test_anisotropic_affine_selected():
    rng = np.random.default_rng(42)
    src_pts = rng.uniform(30.0, 180.0, (60, 2)).astype(np.float32)
    true_M = np.array([[1.20, -0.15, 8.0], [0.10, 0.85, 12.0]], dtype=np.float64)
    ref_pts = apply_transformation(src_pts, true_M)

    model, M, mask, res, reason, evals = select_best_transformation_model(
        src_pts, ref_pts,
        candidate_models=("similarity", "affine", "homography"),
        ransac_threshold=1.0,
    )
    assert model in ("affine", "homography")


def test_random_uncorrelated_points_fail_honestly():
    rng = np.random.default_rng(999)
    src_pts = rng.uniform(0.0, 1000.0, (50, 2)).astype(np.float32)
    ref_pts = rng.uniform(0.0, 1000.0, (50, 2)).astype(np.float32)

    with pytest.raises(RuntimeError) as excinfo:
        select_best_transformation_model(
            src_pts, ref_pts,
            candidate_models=("similarity", "affine", "homography"),
            ransac_threshold=2.0,
        )
    assert "Geometric verification failed" in str(excinfo.value)


def test_collinear_points_fail_safely():
    src_pts = np.array([[float(i), float(i)] for i in range(10)], dtype=np.float32)
    ref_pts = np.array([[float(i * 2), float(i * 2)] for i in range(10)], dtype=np.float32)
    model, M, mask, res, reason, evals = select_best_transformation_model(
        src_pts, ref_pts,
        candidate_models=("affine", "homography"),
        ransac_threshold=2.0,
    )
