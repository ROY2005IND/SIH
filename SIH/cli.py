...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="lunarmatch",
        description="LUNARMATCH AI (formerly MoonFlower AI): Multi-Modal, Sun-Angle and Scale-Invariant Lunar Image Registration (ISRO SIH #26166)",
...
def main() -> int:
    ...
    if not args.quiet and not args.json:
        ...
        print("=" * 72)
        print(" [SYSTEM] LUNARMATCH AI — SCIENTIFIC MISSION REGISTRATION ENGINE")
    ...
    run_id = tracker.record_run(
        ...
        preprocessing_method=args.prep,
        matcher_method=args.matcher,
        notes="Executed via LUNARMATCH Headless CLI",
    )
    ...
    if not args.quiet:
        if result.success and result.metrics and result.quality_score:
            ...
            rmse_disp = f"{m.rmse_px:.3f} px" if m.rmse_px is not None else "N/A"
            med_disp = f"{m.median_residual_px:.3f} px" if m.median_residual_px is not None else "N/A"
            print("\n" + "-" * 72)
            print(f" [OK] REGISTRATION STATUS: SUCCESS (Run ID: {run_id})")
            print(f"   Operating State:    {getattr(result, 'operating_state', 'VERIFIED_GEOSPATIAL')}")
            print(f"   Model Selected:     {result.selected_model.upper()} ({result.model_selection_reason})")
            print(f"   Inlier RMSE:        {rmse_disp} (Median: {med_disp})")
            ...
            state = getattr(result, "operating_state", "REJECTED")
            reason = getattr(result, "rejection_reason", "Unable to verify sufficient spatial correspondences.")
            print(f"\n[FAILED] REGISTRATION REJECTED [{state}]: {reason}\n", file=sys.stderr)
            return 1
