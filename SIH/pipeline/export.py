...
def export_registration_package(
    ...
    rmse_str = f"{result.metrics.rmse_px:.3f} px" if (result.metrics and result.metrics.rmse_px is not None) else "N/A — insufficient verified correspondences (< 4 inliers)"
    med_res_str = f"{result.metrics.median_residual_px:.3f} px" if (result.metrics and result.metrics.median_residual_px is not None) else "N/A"
    
    rep_text = f"""# LUNARMATCH AI — Planetary Scientific Registration Report
**Problem Statement:** ISRO SIH #26166  
**Registration Status:** {'SUCCESSFUL' if result.success else 'REJECTED / FAILED'}  
**Operating State:** `{getattr(result, 'operating_state', 'UNKNOWN')}`  
**Total Pipeline Execution Time:** {result.total_pipeline_time:.3f} seconds  
{f"**Rejection Reason:** {result.rejection_reason}  " if getattr(result, 'rejection_reason', '') else ""}
...
- **Transfer RMSE:** **{rmse_str}**
- **Median Residual Error:** {med_res_str}
...
- **Internal Registration Quality Index:** **{result.quality_score.total_score if result.quality_score else 0} / 100 ({result.quality_score.rating_label if result.quality_score else 'N/A'})**
*Disclaimer: Internal composite heuristic; not an official ISRO accuracy metric.*

---

## 4. Geometric Model Selection & Rejection
...
## 6. Stage-by-Stage Performance Profile

| Stage | Duration (s) |
| :--- | :--- |
{chr(10).join([f"| **{k.replace('_seconds', '').replace('_', ' ').title()}** | {v:.4f} s |" for k, v in getattr(result, 'stage_timings', {}).items()])}
"""
