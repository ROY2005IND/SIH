...
class RasterTiler:
    ...
    def from_file_path(
        ...
        """
        Create a RasterTiler instance streaming tiles via memory-mapped file handles,
        preventing high RAM allocation for large planetary rasters (>1 GB).
        """
        try:
            import tifffile
            memmap_img = tifffile.memmap(file_path, mode="r")
            return cls(image=memmap_img, tile_size=tile_size, overlap_px=overlap_px)
        except Exception:
            # Fallback to standard open if memmap is unsupported for the image format
            from preprocessing.loader import load_lunar_image
            lunar_img = load_lunar_image(file_path)
            return cls(image=lunar_img.image_u8, tile_size=tile_size, overlap_px=overlap_px)
